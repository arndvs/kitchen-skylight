...
async function launch() {
  ...
  await page.waitForSelector('[data-tile-type="clock"]', { timeout: 15000 }) // home is the boot view
  return { app, page }
}
...

{
  ...
  try {
    ...
    // remove meals + chores tiles, opening a 6x2 hole at (4,4)
    ...
    // drag the stars tile (2x2 at 10,4) into the hole at (4,4)
    const { cellCenter, cellW } = await cellMath(page)
    const from = cellCenter(10, 4, 2, 2)
    const to = cellCenter(4, 4, 2, 2)
    await dragTo(page, from.x, from.y, to.x, to.y)
    const movedBox = await page.locator('[data-tile-id="default-starBalances"]').boundingBox()
    const expected = cellCenter(4, 4, 2, 2)
    if (Math.abs(movedBox.x + movedBox.width / 2 - expected.x) > cellW / 2) {
      throw new Error(`drag failed: tile center ${movedBox.x + movedBox.width / 2} vs expected ${expected.x}`)
    }

    // resize it one cell wider via the corner grip
    const widthBefore = movedBox.width
    const grip = await page.locator('[data-tile-id="default-starBalances"] [data-resize-handle]').boundingBox()
    await dragTo(page, grip.x + grip.width / 2, grip.y + grip.height / 2, grip.x + grip.width / 2 + cellW + GAP, grip.y + grip.height / 2)
    const widthAfter = (await page.locator('[data-tile-id="default-starBalances"]').boundingBox()).width
    if (widthAfter - widthBefore < cellW * 0.6) {
      throw new Error(`resize failed: width ${widthBefore} -> ${widthAfter}`)
    }

    // add-tile sheet: singleton weather is disabled, photo can be added
    await page.getByRole('button', { name: 'Add tile' }).click()
    const weatherDisabled = await page.getByRole('button', { name: 'Weather Already added' }).isDisabled()
    if (!weatherDisabled) throw new Error('expected Weather tile entry to be disabled (already placed)')
...
// relaunch with the SAME user data — the layout must have persisted
{
  ...
  try {
    ...
    if (!stars || stars.x !== 4 || stars.y !== 4 || stars.w !== 3 || stars.h !== 2) {
