...
if (-not (Get-LocalUser -Name $KioskUser -ErrorAction SilentlyContinue)) {
  ...
  # A kiosk account needs no password. New-LocalUser requires the -Password
  # parameter but rejects an empty string; pass an empty but valid SecureString
  # so the account is created passwordless (no interactive prompt).
